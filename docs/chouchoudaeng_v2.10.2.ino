...
void loop() {
    ...
    if (digitalRead(pay_pin) == HIGH) {  //결제 되었을 때 만0
        ...
        if (digitalRead(shampoo_add_button) == LOW &&
            shampoo_add_state < 3) {  // LED버튼 누르면 샴푸투입 최대 3회
            {
                Serial.print("샴푸투입");
        ...
        for (int i = 0; i < 9; i++) {  //버튼 받아오기
            ...
            if ((millis() - shower_time_prev) > 10000 &&
                button_toggle[2] == 0 &&
                state == 2) {  //샴푸 누르면 월풀 10초후에 꺼짐
                digitalWrite(relay_pin[3], HIGH);  //월풀끄고
                shampoo_cnt = 0;
            }

            if ((millis() - rinse_time_prev) > 10000 &&
                state == 3) {  //헹굼 누르면 월풀 10초후에 꺼짐
                digitalWrite(relay_pin[3], HIGH);  //월풀끄고
            }
        }
...
void debouncing_button(int i) {
    ...
    if ((millis() - last_debounce[i]) >
        debounce_delay) {  //스위치 상태가 debounce_delay 이상 같으면
...
void state_2(int i) {
    if (state != 2) {  //샴.월.스 처음 킬때
        ...
        if (analogRead(water_pin) < 500) {
            ...
            if ((button_toggle[1] == 0) &&
                (button_toggle[2] == 0)) {  //샴월둘다꺼지면
...
void auto_clean_check() {
    if (auto_clean_queue == 1) {  //대기중이고, 사람없으면
        ...
        if (auto_clean_prev > 0 &&
            (millis() - auto_clean_prev) > 600000) {  // 600초되면
...
void dryer_check() {  //토글에따른 릴레이 제어 부분
    ...
    if (button_toggle[7] == 1 ||
        button_toggle[8] == 1) {  //집진기 둘중하나라도 켜지면 켜기
